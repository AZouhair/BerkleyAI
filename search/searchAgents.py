            ...
            if not hitsWall :
                for corner in state[1]: 
                    if (nextx,nexty) == corner[0]:
                        cornerState.append((corner[0],True))
                    else:
                        cornerState.append(corner)
                successors.append((((nextx,nexty),tuple(cornerState)),action,1))
...
def cornersHeuristic(state, problem):
    ...
    for corner in state[1]:        
        if corner[1] is False:
            distances.append(abs(corner[0][0]-state[0][0])+abs(corner[0][1]-state[0][1]))
...
def foodHeuristic(state, problem):
    ...
    if not food:
        ...
        return max([manhattanDistance(position, foodPos, problem.startingGameState) for foodPos in food])
